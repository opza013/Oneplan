# ONE PLAN v5.4.16 — Detail Gantt 25% Zoom & Weekend Hatch
...
Health check version: `5.4.16-detail-gantt-25-weekend-hatch`.
...
## v5.4.16 Detail Plan Gantt
- Detail Plan Gantt zoom now supports **25%–250%**.
- Detail Gantt uses an independent zoom level so Main Gantt keeps its existing zoom behavior.
- In **Day** view, Saturday and Sunday columns are shaded with a gray diagonal hatch across both the header and activity timeline rows.
- Weekend hatching is visual-only and does not change activity dates or scheduling logic.
